...
from tests.utils.integration_helpers import DtnTestEnvironment, requires_docker
from tests.utils.proc_helpers import run_and_expect_multiple, run_and_expect_single
...
def test_execution_plan_once(dtnd_bde_env: DtnTestEnvironment) -> None:
    ...
    exec_cmd = (
        ...
        f"exec tests/artifacts/execution_plans/execution_plan_once.toml"
    )
...
def test_execution_plan_twice_named(dtnd_bde_env: DtnTestEnvironment) -> None:
    ...
    exec_cmd = (
        ...
        f"-v "
        f"client --results_dir {results_dir} "
        f"exec tests/artifacts/execution_plans/execution_plan_twice_named.toml"
    )
    exec_proc = env.compose_env.popen("client-ns", exec_cmd)
