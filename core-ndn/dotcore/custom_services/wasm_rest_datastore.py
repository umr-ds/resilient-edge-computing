...
class WasmRest(ConfigService):
    ...
    startup: List[str] = ["bash -c 'python3 /root/wasm_rest/wasm_rest.py -v datastore --hosts $(hostname -I | grep -E -o \"[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\" | paste -sd ',') --rootdir /root/wasm_rest/out &> /root/log/$(hostname).log'"]
